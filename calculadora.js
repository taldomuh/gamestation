...
function calcularDesconto() {
    if (!calculadora.validarInputs(['preco-original', 'percentual-desconto', 'quantidade'])) {
    return;
    }
    ...
    detalhes: [
    { label: 'Preço Original', valor: `R$ ${precoOriginal.toFixed(2)}` },
    { label: 'Desconto', valor: `R$ ${valorDesconto.toFixed(2)}` },
    { label: 'Preço Final', valor: `R$ ${precoComDesconto.toFixed(2)}` },
    { label: 'Quantidade', valor: quantidade },
    { label: 'Total a Pagar', valor: `R$ ${totalComDesconto.toFixed(2)}` },
    { label: 'Você Economiza', valor: `R$ ${economiaTotal.toFixed(2)}` }
    ],
    grafico: {
    tipo: 'bar',
